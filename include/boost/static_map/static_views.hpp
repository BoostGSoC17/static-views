...
/// \cond
template <class V> struct is_view<V &> : is_view<V> {};
/// \endcond
...
template <class V> struct is_pipe : std::is_base_of<pipe_base, V> {};

/// \cond
template <class V> struct is_pipe<V &> : is_pipe<V> {};
/// \endcond
...
template <class T> struct wrapper<T &> {
  ...
  constexpr wrapper(T &x) noexcept : _payload{&x} {}
...
template <class T> struct wrapper<T &&> {
  ...
  constexpr auto get() && noexcept -> T { return std::move(_payload); }
...
/// \snippet include/boost/static_map/static_views.hpp make_wrapper
/// implementation
///
...
template <class T> constexpr auto make_wrapper(T &&x) noexcept {
  return detail::wrapper<std::conditional_t<
      std::is_lvalue_reference<decltype(x)>::value, T &, T &&>>{
      std::forward<T>(x)};
}
/// [make_wrapper implementation]
...
namespace detail {
template <class Maker, class... Args> struct pipe_impl : pipe_base {
  constexpr pipe_impl(Maker make, std::tuple<Args...> &&args) noexcept
      : _make{std::move(make)},
        _args{std::move(args)} {}

  template <class View> constexpr auto operator()(View &&xs) const noexcept {
  ...
  template <class View> constexpr auto operator()(View &&xs) noexcept {
    static_assert(is_view<View>::value, "detail::pipe_impl::operator() "
                                        "`View` must model the View concept.");
    return call_impl(std::forward<View>(xs),
                     std::make_index_sequence<sizeof...(Args)>{});
...
private:
  ...
  template <class View, std::size_t... Is>
  constexpr auto call_impl(View &&xs,
                           std::index_sequence<Is...>) const &noexcept {
    return _make(std::get<Is>(_args)..., make_wrapper(std::forward<View>(xs)));
  }
};

template <class Maker, class... Args>
constexpr auto make_pipe_impl(Maker &&make, Args &&... args) noexcept {
  return pipe_impl<Maker, Args...>{
      std::forward<Maker>(make), std::make_tuple(std::forward<Args>(args)...)};
}

template <class Maker> struct pipe_variable_impl {
  constexpr pipe_variable_impl(Maker &&make) noexcept : _make{std::move(make)} {
  }

  template <class... Args>
  constexpr auto operator()(Args &&... args) const &noexcept {
    return make_pipe_impl(_make, make_wrapper(std::forward<Args>(args))...);
  }

  template <class... Args>
      constexpr auto operator()(Args &&... args) && noexcept {
    return make_pipe_impl(std::move(_make),
...
namespace detail {
template <class Predicate> struct not_fn_impl {
  constexpr not_fn_impl(Predicate &&p) noexcept : _p{std::move(p)} {}
...
namespace detail {
template <std::size_t I> struct apply_composed_impl_from {
  template <class Tuple, class... Args>
  constexpr auto operator()(Tuple &&fs, Args &&... args) const noexcept {
    return apply_composed_impl_from<I - 1>{}(
        std::forward<Tuple>(fs), std::get<I>(fs)(std::forward<Args>(args)...));
  }
};

template <> struct apply_composed_impl_from<0> {
  template <class Tuple, class... Args>
  constexpr auto operator()(Tuple &&fs, Args &&... args) const noexcept {
...
template <class... Fs> struct compose_impl {
  constexpr compose_impl(Fs &&... fs) noexcept : _fs{std::move(fs)...} {}
...
template <class... Fs> constexpr auto make_compose_impl(Fs &&... fs) noexcept {
...
template <class... Fs> constexpr auto compose(Fs &&... fs) noexcept {
...
namespace detail {
template <class F, class... Fs> struct mapped_compose_impl {
  constexpr mapped_compose_impl(F &&f, Fs &&... fs) noexcept
      : _f{std::move(f)},
        _fs{compose(std::move(fs)...)} {}
...
constexpr auto mapped_compose(F &&f, _Fs &&... fs) noexcept {
  return detail::make_mapped_compose_impl(
      make_wrapper(std::forward<F>(f)), make_wrapper(std::forward<_Fs>(fs))...);
}
...
struct view_adaptor_core_access {
  ...
  template <class View>
  static constexpr decltype(auto) map(View const &xs,
                                      std::size_t const i) noexcept {
    return xs.map(i);
  }
};

template <class T, class Wrapper> struct view_adaptor_base : view_base {

  constexpr view_adaptor_base(Wrapper &&wrapper) noexcept : _xs{wrapper} {}
  ...
  static constexpr auto capacity() noexcept { return type::capacity(); }
...
template <class Sequence> struct raw_view_impl : view_base {
  constexpr raw_view_impl(Sequence &xs) noexcept : _xs{xs} {}
  ...
  constexpr decltype(auto) at(std::size_t const i) const {
    return (i < size()) ? sequence_traits<Sequence>::at(_xs, i)
                        : (
                              throw std::out_of_range{
                                  "detail::raw_view_impl::at() index `i`"
                                  " out of bounds."},
...
/// \brief Piping.
template <
    class View, class Pipe,
    class = std::enable_if_t<is_view<View>::value && is_pipe<Pipe>::value>>
...
/// \overload
template <
    class Sequence, class Pipe,
    class = std::enable_if_t<!is_view<Sequence>::value && is_pipe<Pipe>::value>>
...
struct drop_impl : view_adaptor_base<drop_impl<View>, View> {
  ...
  friend struct ::view_adaptor_core_access;

  constexpr drop_impl(View &&xs, std::size_t const b) noexcept
      : base_type{std::move(xs)},
        _b{b} {}

  constexpr auto size() const noexcept { return this->_ref().size() - _b; }
...
struct make_drop_impl {
  template <class View>
  constexpr auto operator()(std::size_t const b, View &&xs) const noexcept {
...
struct make_drop_while_impl {
  ...
  constexpr auto operator()(Predicate &&p, View &&xs) const noexcept {
    auto const b = find_first_i(xs.get(), not_fn(std::forward<Predicate>(p)));
...
struct reverse_impl : view_adaptor_base<reverse_impl<View>, View> {
  ...
  constexpr reverse_impl(View &&xs) noexcept : base_type{std::move(xs)} {}
...
struct make_take_impl {
  template <class View>
  constexpr auto operator()(std::size_t const n, View &&xs) const noexcept {
    auto const size = xs.get().size();
    return reverse(std::forward<View>(xs).get()) |
           drop(size - std::min(n, size)) | reverse;
  }
};
} // end namespace detail
...
struct make_slice_impl {
  template <class View>
  constexpr auto operator()(std::size_t const b, std::size_t const e,
...
constexpr auto _make_filter_init_impl(S &&xs, P &&p,
  ...
  return filter_init_impl<Is...>(std::forward<S>(xs), std::forward<P>(p));
}

template <class S, class P>
constexpr auto make_filter_init_impl(S &&xs, P &&p) noexcept {
  constexpr auto capacity = std::remove_reference_t<S>::capacity();
  return _make_filter_init_impl(std::forward<S>(xs), std::forward<P>(p),
                                std::make_index_sequence<capacity>{});
}

template <class V> struct filter_impl : view_adaptor_base<filter_impl<V>, V> {
  friend ::view_adaptor_core_access;

  template <std::size_t... Is>
  constexpr filter_impl(V &&xs, filter_init_impl<Is...> &&init) noexcept
      : base_type{std::move(xs)},
        _n{init._n},
        _is{init._is[Is]...} {}
...
struct make_filter_impl {
  ...
  constexpr auto operator()(Predicate &&p, View &&xs) const noexcept {
    auto init =
        detail::make_filter_init_impl(xs.get(), std::forward<Predicate>(p));
    return detail::filter_impl<View>(std::forward<View>(xs), std::move(init));
  }
};
} // end namespace detail
...
template <std::size_t... Is> struct sort_init_impl {
  std::size_t _is[sizeof...(Is)];

  template <class View, class Predicate>
  constexpr sort_init_impl(View const &xs, Predicate &&p) noexcept
      : _is{Is...} {
    bubble_sort_impl(_is, xs.size(), mapped_compose(std::forward<Predicate>(p),
                                                    view_at<View>{xs}));
  }

  template <class View> struct view_at {
    View const &_xs;
...
template <class V> struct sort_impl : view_adaptor_base<sort_impl<V>, V> {
  friend struct ::view_adaptor_core_access;
...
struct make_sort_impl {
  ...
  constexpr auto operator()(Predicate &&p, View &&xs) const noexcept {
    auto init =
        detail::make_sort_init_impl(xs.get(), std::forward<Predicate>(p));
...
template <std::size_t... Is> struct hashed_init_impl {
...
private:
  constexpr auto insert(std::size_t const i, std::size_t const guess) noexcept {
    std::size_t indent = find_first_i(raw_view(_is) | drop(guess), empty);
    if (guess + indent < capacity) {
      _is[guess + indent] = i;
      return;
    }

    indent = find_first_i(raw_view(_is) | take(guess), empty);
    _is[indent] = i;
  }
...
struct hashed_impl : view_adaptor_base<hashed_impl<Capacity, V, Hasher>, V> {
  ...
  constexpr decltype(auto) hash_function() const noexcept { return _hf.get(); }

private:
  std::size_t _is[capacity()];
  Hasher _hf;
  using base_type = view_adaptor_base<hashed_impl<Capacity, V, Hasher>, V>;
...
template <std::size_t Capacity> struct make_hashed_impl {
  template <class View, class Hasher>
  constexpr auto operator()(Hasher &&h, View &&xs) const noexcept {
    auto hf = make_wrapper(std::forward<Hasher>(h));
    auto init = detail::make_hashed_init_impl<Capacity>(xs.get(), hf.get());
...
constexpr auto
    hashed = make_pipe_variable(detail::make_hashed_impl<Capacity>{});
