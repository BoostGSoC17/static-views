...
template <class DropTake> struct slice_impl : DropTake {
  ...
  static_assert(std::is_same<DropTake, std::decay_t<DropTake>>::value,
...
private:
  ...
  template <class T> static constexpr auto check_has_parent() noexcept -> bool {
    return is_detected<call_parent_t, T>::value;
  }

  template <class T, class = std::enable_if_t<check_has_parent<T>()>,
            class = void>
  static constexpr auto check_parent_returns_view() noexcept -> bool {
    return concepts::is_View<std::decay_t<detected_t<call_parent_t, T>>>();
  }
  ...
  template <class T> static constexpr auto check_parent() noexcept -> bool {
    return utils::all(check_has_parent<T>(), check_parent_returns_view<T>());
  }

  template <class T> static constexpr auto assert_parent() noexcept -> bool {
    constexpr bool x = check_parent<T>();
    static_assert(x, "[INTERNAL] `T` is required to have a `parent()` member "
...
public:
  ...
  template <class View,
            ...
            class = std::enable_if_t<
                !std::is_same<std::decay_t<View>, slice_impl<DropTake>>::value>>
  ...
  map(std::size_t const i) const BOOST_STATIC_VIEWS_NOEXCEPT_IF(
      noexcept(std::declval<DropTake const &>().parent().map(
          std::declval<DropTake const &>().map(std::declval<std::size_t>())))) {
    static_assert(noexcept(std::declval<DropTake const &>().parent()),
  ...
  BOOST_STATIC_VIEWS_DECLTYPE_AUTO parent() const noexcept {
    static_assert(noexcept(std::declval<DropTake const &>().parent()),
                  "[INTERNAL] Why is parent() not noexcept?");
    using parent_type =
        std::decay_t<decltype(std::declval<DropTake const &>().parent())>;
