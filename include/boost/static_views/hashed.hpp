...
namespace detail {
    ...
    private:
        ...
        BOOST_FORCEINLINE
        BOOST_STATIC_VIEWS_CONSTEXPR auto insert(std::size_t const i,
            std::size_t const guess) -> void
        {
            struct is_empty {
        ...
        BOOST_STATIC_VIEWS_CONSTEXPR hashed_impl(View&& xs, HashFunction&& hf,
            ...
            BOOST_STATIC_VIEWS_NOEXCEPT_IF(utils::all(
                std::is_nothrow_constructible<typename
                    hashed_impl::view_adaptor_base_type, View&&>::value,
    ...
    struct make_hashed_impl {
           ...
           ))
        {
            ...
            return detail::hashed_impl<BucketCount, BucketSize, 
                std::decay_t<View>, std::decay_t<Hasher>>{
                std::forward<View>(xs), std::forward<Hasher>(hf), init.storage,
                std::make_index_sequence<BucketCount * BucketSize>{}};
        }
    };
