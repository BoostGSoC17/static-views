...
/// By deriving from :cpp:class:`algorithm_base`, you tell StaticView that
/// the
...
#else
        BOOST_STATIC_VIEWS_NOEXCEPT_IF(utils::all(
            std::is_nothrow_move_constructible<std::tuple<wrapper<Args>...>>::
                value,
            std::is_nothrow_move_constructible<Function>::value))
#endif
...
#define FAKE_CALL_IMPL(qualifiers)                                             \
    ...
    template <class View>
    BOOST_FORCEINLINE
        BOOST_STATIC_VIEWS_CONSTEXPR BOOST_STATIC_VIEWS_DECLTYPE_AUTO
        operator()(View&& xs) const & BOOST_STATIC_VIEWS_NOEXCEPT_IF(
            noexcept(FAKE_CALL_IMPL(const&)))
    {
  ...
  private:
    ...
    static constexpr auto is_noexcept_call_move() noexcept -> bool
    {
        return noexcept(invoke(std::declval<Function&&>(),
            make_wrapper(std::declval<View&&>()), std::declval<Args&&>()...));
    }

    template <class View, std::size_t... Is>
    BOOST_FORCEINLINE
        BOOST_STATIC_VIEWS_CONSTEXPR BOOST_STATIC_VIEWS_DECLTYPE_AUTO
        call_impl(View&& xs, std::index_sequence<Is...>)
            const & BOOST_STATIC_VIEWS_NOEXCEPT_IF(
                is_noexcept_call_cref<View&&>())
    {
    ...
    */
    ...
    {
        return invoke(std::move(_func), make_wrapper(std::forward<View>(xs)),
...
struct make_algorithm_impl {
    ...
    BOOST_FORCEINLINE BOOST_STATIC_VIEWS_CONSTEXPR auto operator()(
        ...
        BOOST_STATIC_VIEWS_NOEXCEPT_IF(
            noexcept(algorithm_impl<Function, Args&&...>(
...
#define BOOST_STATIC_VIEWS_INLINE_ALGO_VARIABLE(type, name)                    \
    inline namespace {                                                         \
        BOOST_STATIC_VIEWS_CONSTEXPR auto const& name =                        \
            ::BOOST_STATIC_VIEWS_NAMESPACE::                                   \
                _static_const<::BOOST_STATIC_VIEWS_NAMESPACE::detail::         \
