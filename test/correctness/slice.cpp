...
auto create_impl(std::index_sequence<Is...>, std::index_sequence<Js...>)
{
    ...
    BOOST_ATTRIBUTE_UNUSED CONSTEXPR decltype(boost::static_views::slice(
        ...
        raw1)) v1[] = {boost::static_views::slice(Js / N, Js % N)(raw1)...};
    ...
    BOOST_ATTRIBUTE_UNUSED CONSTEXPR decltype(boost::static_views::slice(
        ...
        boost::static_views::slice(Js / N, Js % N)(decltype(raw2){raw2})...};
}

template <class T, std::size_t N, std::size_t M>
auto test_create()
{
    return create_impl<T, M>(
        std::make_index_sequence<N>{}, std::make_index_sequence<M * M>{});
}
...
auto test_size()
{
    return size_impl<T, M>(
        std::make_index_sequence<N>{}, std::make_index_sequence<M * M>{});
}

template <std::size_t B, std::size_t E, std::size_t... Is, std::size_t... Js>
auto access_impl(std::index_sequence<Is...>, std::index_sequence<Js...>) -> void
{
    ...
    BOOST_TEST_TRAIT_TRUE(
        (std::is_same<decltype(boost::static_views::slice(B, E)(raw1)[0]),
            std::size_t const&>));
    // Run-time access
    for (std::size_t j = 0; j < sizeof...(Js); ++j) {
...
{
    BOOST_ATTRIBUTE_UNUSED int dummy[] = {
        ((void)test_access_impl_2<N, M, Is>(std::make_index_sequence<Is>{}),
            0)...};
}
...
int main(void)
{
    ...
    std::cout << "1 --> " << boost::static_views::slice(1ul, 3ul)(v).map(1)
              << '\n';
