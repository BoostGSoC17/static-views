...
int main(void)
{
    ...
    CONSTEXPR_AFTER_CXX14 auto ds_view =
    ...
    BOOST_TEST_TRAIT_TRUE(
        (std::is_same<int const&, decltype(bs_view[2])>));
    BOOST_TEST_TRAIT_TRUE(
        (std::is_same<int const&, decltype(cs_view[2])>));
    BOOST_TEST_TRAIT_TRUE(
        (std::is_same<int const&, decltype(ds_view[0])>));
