...
template <class _Char>
inline constexpr auto crc32_hash(_Char const* const str) noexcept
    -> std::uint32_t
{
...
struct hash_impl<std::experimental::string_view> {
    constexpr auto operator()(std::experimental::string_view const& x) const
        noexcept -> std::size_t
    {
...
struct hash_c {
    ...
    BOOST_STATIC_VIEWS_CONSTEXPR auto operator()(T&& x) const
        BOOST_STATIC_VIEWS_NOEXCEPT_IF(
            noexcept(hash_impl<std::decay_t<T>>{}(std::forward<T>(x))))
    {
        return hash_impl<std::decay_t<T>>{}(std::forward<T>(x));
    }
};

template <class HashedView, class Pred>
class static_map : private HashedView {
  ...
  private:
    ...
    BOOST_FORCEINLINE static BOOST_STATIC_VIEWS_CONSTEXPR
        BOOST_STATIC_VIEWS_DECLTYPE_AUTO
        _get_key(ValueType&& x) BOOST_STATIC_VIEWS_NOEXCEPT_IF(
            noexcept(std::get<0>(std::forward<ValueType>(x))))
    {
    ...
    BOOST_FORCEINLINE static BOOST_STATIC_VIEWS_CONSTEXPR
        BOOST_STATIC_VIEWS_DECLTYPE_AUTO
        _get_value(ValueType&& x) BOOST_STATIC_VIEWS_NOEXCEPT_IF(
            noexcept(std::get<1>(std::forward<ValueType>(x))))
    {
    ...
    template <class Key>
    BOOST_FORCEINLINE BOOST_STATIC_VIEWS_CONSTEXPR auto _lookup(
        Key const& k) const
        ...
        BOOST_STATIC_VIEWS_NOEXCEPT_IF(boost::static_views::detail::utils::all(
            noexcept(std::declval<key_equal const&>()(
                std::declval<Key const&>(), std::declval<Key const&>())),
            noexcept(std::declval<hasher const&>()(
                std::declval<Key const&>())))) -> value_type*
    {
        struct pred_equal {
            key_equal const& eq;
            Key const&       x;
            ...
            BOOST_STATIC_VIEWS_CONSTEXPR auto operator()(value_type const& y)
                BOOST_STATIC_VIEWS_NOEXCEPT_IF(noexcept(
                    std::declval<key_equal const&>()(std::declval<Key const&>(),
        ...
        auto const i =
            boost::static_views::find_first_i(ys, pred_equal{_eq, k});
  ...
  public:
    ...
    BOOST_STATIC_VIEWS_CONSTEXPR static_map(view_type&& xs, KeyEqual&& eq)
        ...
        : view_type{std::move(xs)}, _eq{std::forward<KeyEqual>(eq)}
    {
    }

    BOOST_STATIC_VIEWS_CONSTEXPR auto size() const
        BOOST_STATIC_VIEWS_NOEXCEPT_IF(
            noexcept(std::declval<view_type const&>().size())) -> size_type
    {
        return static_cast<view_type const*>(this)->size();
    }

    template <class Key>
    BOOST_FORCEINLINE BOOST_STATIC_VIEWS_CONSTEXPR auto find(Key const& k) const
        BOOST_STATIC_VIEWS_NOEXCEPT_IF(
            noexcept(std::declval<static_map const&>()._lookup(
                std::declval<Key const&>()))) -> value_type*
    {
        return _lookup(k);
    }
    ...
    template <class Key>
    BOOST_FORCEINLINE BOOST_STATIC_VIEWS_CONSTEXPR auto
    count(Key const& k) const BOOST_STATIC_VIEWS_NOEXCEPT_IF(noexcept(
        std::declval<static_map const&>()._lookup(std::declval<Key const&>())))
        -> size_type
    {
    ...
    template <class Key>
    BOOST_FORCEINLINE BOOST_STATIC_VIEWS_CONSTEXPR auto at(Key const& k) const
        -> mapped_type&
    {
        auto* x = _lookup(k);
        return x != nullptr ? _get_value(*x)
                            : (throw key_not_found_error{}, _get_value(*x));
    }

    template <class Key>
    BOOST_FORCEINLINE BOOST_STATIC_VIEWS_CONSTEXPR auto operator[](
        Key const& k) const -> mapped_type&
    {
        return at(k);
    }
};
...
BOOST_STATIC_VIEWS_CONSTEXPR auto make_static_map(
    View&& xs, KeyEqual&& equal = KeyEqual{}, Hasher&& hasher = Hasher{})
{
    using std::get;
    using value_type =
        std::remove_reference_t<decltype(xs[std::declval<std::size_t>()])>;

    struct good_hasher {
        Hasher hash_function;

        BOOST_STATIC_VIEWS_CONSTEXPR auto operator()(value_type const& x) const
            BOOST_STATIC_VIEWS_NOEXCEPT_IF(noexcept(boost::static_views::invoke(
                std::declval<Hasher const&>(),
                std::get<0>(std::declval<value_type const&>())))) -> std::size_t
        {
    ...
    return static_map<decltype(view), KeyEqual>(
        std::move(view), std::forward<KeyEqual>(equal));
}
