...
namespace detail {
    ...
    class static_map : private HashedView {
      ...
      private:
        template <class T>
        using callable_with_value_t = decltype(
            boost::static_views::invoke(std::declval<T const&>(),
                std::declval<value_type>()));

        static_assert(boost::static_views::detail::is_detected<
                          callable_with_value_t, key_getter>::value,
            BOOST_STATIC_VIEWS_BUG_MESSAGE);
        static_assert(boost::static_views::detail::is_detected<
                          callable_with_value_t, value_getter>::value,
            BOOST_STATIC_VIEWS_BUG_MESSAGE);

      public:
        using key_type    = std::remove_reference_t<decltype(
            boost::static_views::invoke(
                std::declval<key_getter const&>(),
                std::declval<value_type&>()))>;
        using mapped_type = std::remove_reference_t<decltype(
            boost::static_views::invoke(
                std::declval<value_getter const&>(),
                std::declval<value_type&>()))>;
        using key_equal   = typename Pred::type;

      private:
        template <class T>
        using callable_with_key_t = decltype(
            boost::static_views::invoke(std::declval<T const&>(),
                std::declval<key_type const&>()));

        template <class T>
        using callable_with_2_key_t = decltype(
            boost::static_views::invoke(std::declval<T const&>(),
                std::declval<key_type const&>(),
                std::declval<key_type const&>()));

        static_assert(boost::static_views::detail::is_detected<
                          callable_with_2_key_t, key_equal>::value,
            BOOST_STATIC_VIEWS_BUG_MESSAGE);
      ...
      public:
        ...
        static_map(view_type&& xs, Pred&& eq, GetKey&& get_key,
            ...
            , _get_mapped{std::move(get_mapped)}
        {
        }
