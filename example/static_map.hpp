...
inline constexpr auto length_c(_Char const* const str) noexcept
    -> std::size_t
{
    std::size_t i = 0;
    while (str[i] != _Char{}) {
...
struct hash_impl<char const*> {
    constexpr auto operator()(char const* const x) const noexcept
        -> std::size_t
    {
...
struct hash_impl<std::experimental::string_view> {
    constexpr auto operator()(
        std::experimental::string_view const& x) const noexcept
        -> std::size_t
    {
...
class static_map : private HashedView {
  ...
  private:
    ...
    BOOST_FORCEINLINE BOOST_STATIC_VIEWS_CONSTEXPR auto _lookup(
        Key const& k) const
        // I know the following specification is formally wrong, but
  ...
  public:
    ...
    BOOST_STATIC_VIEWS_CONSTEXPR static_map(
        view_type&& xs, KeyEqual&& eq)
        BOOST_STATIC_VIEWS_NOEXCEPT_IF(
            boost::static_views::detail::utils::all(
    ...
    BOOST_STATIC_VIEWS_CONSTEXPR auto size() const
        BOOST_STATIC_VIEWS_NOEXCEPT_IF(noexcept(
            std::declval<view_type const&>().size())) -> size_type
    {
        return static_cast<view_type const*>(this)->size();
    }

    template <class Key>
    BOOST_FORCEINLINE BOOST_STATIC_VIEWS_CONSTEXPR auto find(
        Key const& k) const
        BOOST_STATIC_VIEWS_NOEXCEPT_IF(
    ...
    template <class Key>
    BOOST_FORCEINLINE BOOST_STATIC_VIEWS_CONSTEXPR auto count(
        Key const& k) const
        BOOST_STATIC_VIEWS_NOEXCEPT_IF(
            noexcept(std::declval<static_map const&>()._lookup(
                std::declval<Key const&>()))) -> size_type
    {
        return _lookup(k) == nullptr ? 0 : 1;
    }

    template <class Key>
    BOOST_FORCEINLINE BOOST_STATIC_VIEWS_CONSTEXPR auto at(
